...
| T-06 | Reexportar Scopus con fecha de corte declarada | 2 | **Sigue abierto tras la reexportación del 2026-09-08** (`D-563`). La reexportación existe y trae la ventana 2020-2025 con 1342 registros, pero el CSV nativo de Scopus **no trae filas de metadatos y no declara fecha de corte**: comprobado sobre `data/raw/Scopus_Sept.csv`, cuya primera línea ya es la cabecera de columnas. El 2026-08-30 que sí se declara es de SciVal y no se traslada (`D-260`, `D-261`, `docs/UPDATING_REQUEST.md` §5), así que `scopus_export.fecha_corte` sigue `null`. Se cierra cuando la fuente entregue un export que la declare, o cuando se documente que este formato nunca lo hará y se sustituya el requisito por la verificación por API de `T-20` |
...
| T-23 | Resolver el presupuesto de peso de datos, excedido por la carga 2020-2025 | 1 | **Abierto el 2026-09-09**. `node src/verify/peso.mjs` sobre el `dist/` de esta carga: datos **369,6 KB comprimidos de un techo de 300 KB, 123 %**, en 15 archivos y 2.077 KB en bruto. CSS (38,3 de 60) y JavaScript (79,4 de 150) siguen holgados. La causa es directa: el corpus pasó de 823 a 1.342 publicaciones y de 530 a 829 fichas de autor. El propio verificador lo dice: subir el techo es una decisión y exige declarar contra qué evidencia. **El despliegue está detenido hasta que se decida**: `.github/workflows/deploy.yml` corre `src/verify/run_all.mjs` dentro del job `construir`, sin `continue-on-error`, y el job `desplegar` depende de él, así que un presupuesto excedido bloquea la publicación. Las salidas reales son dos: subir el techo declarando contra qué evidencia, o aligerar los artefactos (paginar o partir `publications.json`, que es el que manda). Ninguna se toma sola |
