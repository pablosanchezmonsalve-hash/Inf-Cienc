...
| T-04 | Revisión humana de los 20 nombres con múltiples Scopus ID, de los que **10 afectan al informe** | 2 | **Desbloqueado**: `internal/revision_identidad.html` (`make revision`). La auditoría mide ahora dos hechos por caso: si la firma está en la población UFT —10 son coautores externos sin ficha ni efecto en ninguna cifra— y si los dos identificadores coocurren en una publicación, que descarta «perfil fragmentado» (pasa en `Lin, John C.`). No deciden la ambigüedad; ordenan la cola |
...
| T-06 | Reexportar Scopus con fecha de corte declarada | 2 | **Conector implementado**: `src/enrich/scopus_api.py` (`make scopus`). Falta ejecutar la consulta — exige `SCOPUS_API_KEY` en el entorno del usuario |
...
| T-10 | Red de coautoría autor–autor derivada de `Autoria` | V2 | **El grafo ya se construye**: `src/build/grafo_coautoria.py` (538 personas, 610 pares, componentes y comunidades de Louvain, sin dependencias externas) y `make red` dibuja la vista real en `internal/red_coautoria.html`, con las mismas primitivas que usará el sitio. Sigue sin publicarse porque depende de T-03: una persona partida en dos nodos hace que la figura afirme que dos investigadores no colaboran. La vista marca los candidatos y sirve para resolver T-03 |
...
| T-14 | Revisión humana de los grupos de firmas que comparten ORCID (**10 casos, 2 pendientes**) | V2 | **Desbloqueado**: `make revision`; evidencia reforzable con `make verificar-orcid`. Eran 17 casos: los que sólo se diferenciaban en diacríticos ya no se preguntan. Cada caso muestra con qué respaldo entró el identificador en cada firma —fuente, confianza y publicaciones— porque el vínculo con Crossref se hace por apellido e inicial dentro de una publicación, así que dos firmas con apellidos distintos que comparten identificador no pudieron coincidir por el nombre |
