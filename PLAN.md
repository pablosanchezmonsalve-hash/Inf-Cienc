...
| T-24 | Confirmar los términos de licencia de Clarivate/Web of Science | 3 | **Abierto el 2026-09-14**. `docs/DATA_LICENSE.md` sólo cubre Elsevier (Scopus/SciVal); no menciona Clarivate en ningún punto, y sus términos no se pueden asumir iguales. `D-599` publica el identificador `UT` de WoS en el sitio mientras esto sigue sin confirmar, con la salvedad declarada explícitamente junto al dato. Si Clarivate resulta más restrictivo, `UT` se retira de lo publicado, no se deriva de nuevo |
